...
{
    sql.getRecentPosts(8).then((sqlData)=>
    {  
        ...
        {
            links_urls += `
                <a xlink:href="${getURL(sqlData[i])}">
                    <text fill="#000000" stroke="#000" stroke-width="0" stroke-opacity="null" x="65.5" y="${73.5 + 33 * i}" id="svg_7" font-size="20" font-family="Oswald, sans-serif" text-anchor="start" xml:space="preserve" font-weight="normal">- ${sqlData[i].name}</text>
                </a>
            `;
        }

        var res = `
        <svg width="800" height="325" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            <g>
                <title>background</title>
                <rect x="-1" y="-1" width="808" height="325" id="canvas_background" fill="#fff"/>
